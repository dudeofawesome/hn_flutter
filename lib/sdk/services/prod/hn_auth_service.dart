...
class HNAuthServiceProd implements HNAuthService {
  ...
  Future<bool> addAccount(String userId, String userPassword) async {
    ...
    return req.close().then((res) {
      ...
      final email =
          doc.querySelector('input[name=uemail]')?.attributes['value'];
      ...
      final topColor = topColorEl != null
          ...
          : null;
      final showDeadEl = doc
          .querySelector('select[name=showd]')
          ?.querySelector('option[selected]');
      final showDead =
          showDeadEl != null ? (showDeadEl.attributes['value'] == 'yes') : null;
      final noProcrastinateEl = doc
          .querySelector('select[name=nopro]')
          ?.querySelector('option[selected]');
