...
class HNUserServiceProd implements HNUserService {
  ...
  Future<Null> init() async {
    assert(this._sendPort == null,
        'HNUserServiceProd::init has already been called');
        ...
        case _IsolateMessageType.GET_USER_BY_ID:
          new Future(() async {
            final user = await http
                .get('${_config.url}/user/${data.params}.json')
                .then((res) => json.decode(res.body))
                .then((body) async {
              if (body != null)
                return new HNUser.fromMap(body);
              else
                return _getUserByIdFromSite(data.params);
            });
            replyTo.send(user);
          });
          break;
  ...
  static Future _getUserByIdFromSite(String id) {
    return http.get('${_config.apiHost}/user?id=$id').then((res) {
      if (res.statusCode != 200) throw 'User "$id" not found';
      return res.body;
    }).then((body) {
      if (body == 'No such user.')
        throw 'User "$id" not found';
      else {
        final created =
            (DateTime.parse(_createdRegExp.firstMatch(body)?.group(1))
  ...
  Future<List<int>> getSavedByUserID(
      String id, bool stories, Cookie accessCookie) async {
  ...
  Future<List<int>> getVotedByUserID(
      String id, bool stories, Cookie accessCookie) async {
    final req = await ((await _httpClient.getUrl(Uri.parse(
...
final _storyIdRegExp =
    new RegExp(r'''<tr class=["']athing["'] id=["']([0-9]+)["']>''');
final _commentIdRegExp =
    new RegExp(r'''<tr class=["']athing["'] id=["']([0-9]+)["']>''');

final _createdRegExp =
    new RegExp(r'''created:.*?<td><a href="front\?day=([0-9\-]+)&birth=''');
final _karmaRegExp =
    new RegExp(r'''<td valign="top">karma:<\/td><td>\s*([0-9]+)\s*<\/td>''');
final _aboutRegExp =
    new RegExp(r'''<td valign="top">about:<\/td><td>\s*(.*)\s*<\/td>''');
