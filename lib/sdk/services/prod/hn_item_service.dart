...
class HNItemServiceProd implements HNItemService {
  ...
  Future<Null> init() async {
    assert(this._sendPort == null,
        'HNItemServiceProd::init has already been called');
        ...
        case _IsolateMessageType.GET_ITEM_BY_ID:
          new Future(() async {
            ...
            if (accessCookie != null) {
              statusUpdates =
                  _parseAllItems(await _getItemPageById(id, accessCookie));
            }

            final item = await http
                .get('${_config.url}/item/$id.json')
          ...
          break;
        case _IsolateMessageType.GET_COMMENT_ITEM_AUTH_BY_ID:
          new Future(() async {
            final int id = data.params['id'];
            final Cookie accessCookie = data.params['accessCookie'];
            ...
            if (accessCookie != null) {
              statusUpdates =
                  _parseAllItems(await _getItemReplyPageById(id, accessCookie));
            }
  ...
  Future<List<HNItemStatus>> getStoryItemAuthById(
      int id, Cookie accessCookie) async {
    if (_itemStore.items[id] == null) {
      addHNItem(new HNItem(id: id));
      patchItemStatus(new HNItemStatus.patch(id: id, loading: true));
    }
    ...
    return statusUpdates..forEach((patch) => patchItemStatus(patch));
  }
  ...
  Future<List<HNItemStatus>> getCommentItemAuthById(
    ...
    this._sendPort.send([
      new _IsolateMessage(
        type: _IsolateMessageType.GET_COMMENT_ITEM_AUTH_BY_ID,
        params: {'id': id, 'accessCookie': accessCookie},
      ),
      response.sendPort
    ]);
  ...
  static Future<String> _getItemPageById(
      int itemId, Cookie accessCookie) async {
    final req =
        await (await _httpClient.getUrl(Uri.parse('${_config.apiHost}/item'
                '?id=$itemId'))
  ...
  static Future<String> _getItemReplyPageById(
      int itemId, Cookie accessCookie) async {
    final req =
        await (await _httpClient.getUrl(Uri.parse('${_config.apiHost}/reply'
                '?id=$itemId'))
              ..cookies.add(accessCookie))
            .close();

    final body =
        await req.transform(utf8.decoder).toList().then((body) => body.join());
  ...
  static HNItemAuthTokens _parseItemAuthTokens(int itemId, String itemPage) {
    return new HNItemAuthTokens(
      ...
      upvote: new RegExp(r'''<a.*?id=["']up_'''
              '$itemId'
              r'''["'].*?href=["']vote\?.*?auth=(.*?)(?:&.*?)?["'].*?>''')
          .firstMatch(itemPage)
          ?.group(1),
      downvote: new RegExp(r'''<a.*?id=["']down_'''
              '$itemId'
              r'''["'].*?href=["']vote\?.*?auth=(.*?)(?:&.*?)?["'].*?>''')
          .firstMatch(itemPage)
          ?.group(1),
      hide: new RegExp(r'''href=["']hide\?.*?id='''
              '$itemId'
              '''&.*?auth=(.*?)["'].*?>(?:un-)?hide''')
          .firstMatch(itemPage)
          ?.group(1),
      save: new RegExp(r'''href=["']fave\?.*?id='''
              '$itemId'
              '''&.*?auth=(.*?)["'].*?>''')
          .firstMatch(itemPage)
          ?.group(1),
  ...
  static HNItemStatus _parseItemStatus(int itemId, String itemPage) {
    return new HNItemStatus.patch(
      id: itemId,
      upvoted: new RegExp(r'''<a.*?id=["']up_'''
                  '$itemId'
                  r'''["'].*?class=["'].*?nosee.*?["'].*?>''')
              .firstMatch(itemPage) !=
          null,
      downvoted: new RegExp(r'''<a.*?id=["']down_'''
                  '$itemId'
                  r'''["'].*?class=["'].*?nosee.*?["'].*?>''')
              .firstMatch(itemPage) !=
          null,
      hidden: new RegExp(r'''href=["']hide\?.*?id='''
                  '$itemId'
                  '''(?:&.*?)?["'].*?>un-hide''')
              .firstMatch(itemPage) !=
          null,
      saved: new RegExp(r'''href=["']fave\?.*?id='''
                  '$itemId'
                  '''(?:&.*?)?["'].*?>un-favorite''')
              .firstMatch(itemPage) !=
          null,
  ...
  static List<HNItemStatus> _parseAllItems(String itemPage) {
    ...
    final itemIds =
        upvoteLinks.map((match) => int.parse(match.id.substring(3)));

    final patches = itemIds
        .map((id) =>
            new HNItemStatus.patch(id: id, authTokens: new HNItemAuthTokens()))
        .toList();
    patches.forEach((patch) {
      final upvote = upvoteLinks.firstWhere((a) => a.id == 'up_${patch.id}',
          orElse: () {});
      final downvote = downvoteLinks
          .firstWhere((a) => a.id == 'down_${patch.id}', orElse: () {});
      final hide = hideLinks.firstWhere(
          (a) => a.attributes['href'].startsWith('hide?id=${patch.id}'),
          orElse: () {});
      final fave = faveLinks.firstWhere(
          (a) => a.attributes['href'].startsWith('fave?id=${patch.id}'),
          orElse: () {});
      final reply = replyLinks.firstWhere(
          (input) =>
              input.parent.children.firstWhere(
                  (i) =>
                      i.attributes['name'] == 'parent' &&
                      i.attributes['value'] == '${patch.id}',
                  orElse: () {}) !=
              null,
          orElse: () {});
      ...
      if (upvote != null) {
        ...
        patch.authTokens.upvote =
            new RegExp(r'vote\?id=' '${patch.id}' '(?:&.*?)?auth=(.+?)&')
                .firstMatch(upvote.attributes['href'])
                ?.group(1);
      }
      if (downvote != null) {
        ...
        patch.authTokens.downvote =
            new RegExp(r'vote\?id=' '${patch.id}' '(?:&.*?)?auth=(.+?)&')
                .firstMatch(downvote.attributes['href'])
                ?.group(1);
      }
      if (hide != null) {
  ...
  Future<Null> faveItem(HNItemStatus status, HNAccount account) async {
    ...
    try {
      ...
      final body = await req
          ...
          .toList()
          .then((body) => body.join());
      var faved = new RegExp(r'''href=["']fave\?.*?id='''
                  '${status.id}'
                  '''(?:&.*?)?["'].*?>un-favorite''')
              .firstMatch(body) !=
          null;
  ...
  Future<Null> voteItem(bool up, HNItemStatus status, HNAccount account) {
    ...
    if (up && !(status.upvoted ?? false)) {
      ...
      how = 'down';
    } else if ((up && (status.upvoted ?? false)) ||
  ...
  Future<int> replyToItemById(int parentId, String comment, String authToken,
      Cookie accessCookie) async {
    final req = await ((await _httpClient
            .postUrl(Uri.parse('${_config.apiHost}/comment')))
          ..cookies.add(accessCookie)
          ..headers.contentType = new ContentType(
              'application', 'x-www-form-urlencoded',
              charset: 'utf-8')
          ..write('parent=$parentId'
    ...
    if (req.headers.value('location') == 'deadlink')
      throw 'Outdated hmac (I think)';
    if (req.headers.value('location').contains('fnop=story-toofast'))
      throw 'Submitting too fast';
    if (!req.headers.value('location').startsWith('item?id=')) {
      if (req.headers.value('location').contains('fnop=commconfirm'))
    ...
    return int.parse(
        req.headers.value('location').replaceFirst('item?id=', ''));
  }
  ...
  Future<int> postItem(
    ...
    final req = await ((await _httpClient
            .postUrl(Uri.parse('${_config.apiHost}/r')))
          ..cookies.add(accessCookie)
          ..headers.contentType = new ContentType(
              'application', 'x-www-form-urlencoded',
              charset: 'utf-8')
          ..write('fnid=$authToken'
              '&fnop=submit-page'
              '&title=$title'
    ...
    if (body.contains('Bad login'))
      throw 'Bad login.';
        ...
        .contains('''You're posting too fast. Please slow down. Thanks.'''))
      throw '''You're posting too fast. Please slow down. Thanks.''';
    else if (req.statusCode != 302)
      throw 'Unknown error';
    else if (req.headers.value('location') == null)
      throw 'Unknown error';
    else if (req.headers.value('location') == 'deadlink')
      throw 'Outdated fnid (I think)';
    else if (req.headers.value('location').contains('fnop=story-toofast'))
      throw 'Submitting too fast';

    print('''req.headers->location''');
    print(req.headers.value('location'));

    final newestReq = await ((await _httpClient.getUrl(
            Uri.parse('${_config.apiHost}/${req.headers.value('location')}')))
          ..cookies.add(accessCookie))
        .close();
    final newestBody = await newestReq
        .transform(utf8.decoder)
        .toList()
        .then((body) => body.join());
  ...
  Future<String> getSubmissionAuthToken(Cookie accessCookie) async {
    final req =
        await (await _httpClient.getUrl(Uri.parse('${_config.apiHost}/submit'))
              ..cookies.add(accessCookie))
            .close();
...
final _postedItemId = new RegExp(
    r'''<tr class="athing" id="([0-9]*?)">.*?<font color="#ff6600">*</font>''');
