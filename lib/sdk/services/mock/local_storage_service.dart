...
class LocalStorageServiceMock implements LocalStorageService {
  static final LocalStorageServiceMock _singleton =
      new LocalStorageServiceMock._internal();
  ...
  Future<Null> init() async {
    ...
    this._databases[KEYS_DB] = await openDatabase(keysPath, version: 1,
        onCreate: (Database db, int version) async {
      print('CREATING KEYS TABLE');
      await db.execute(
          'CREATE TABLE $KEYS_TABLE ($KEYS_ID TEXT PRIMARY KEY, $KEYS_VALUE TEXT)');
    });

    String accountsPath = join(this._documentsDirectory.path, ACCOUNTS_DB);
    this._databases[ACCOUNTS_DB] = await openDatabase(accountsPath, version: 1,
        onCreate: (Database db, int version) async {
