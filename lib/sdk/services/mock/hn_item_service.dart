...
class HNItemServiceMock implements HNItemService {
  ...
  Future<HNItem> getItemByID(int id, [Cookie accessCookie]) {
    ...
    return http
        .get('${this._config.url}/item/$id.json')
  ...
  Future<String> _getItemPageById(int itemId, Cookie accessCookie) async {
    final req =
        await (await _httpClient.getUrl(Uri.parse('${this._config.apiHost}/item'
                '?id=$itemId'))
  ...
  Future<String> _getItemReplyPageById(int itemId, Cookie accessCookie) async {
    final req = await (await _httpClient
            .getUrl(Uri.parse('${this._config.apiHost}/reply'
                '?id=$itemId'))
          ..cookies.add(accessCookie))
        .close();
  ...
  List<HNItemStatus> _parseAllItems(String itemPage) {
    ...
    patches.forEach((patch) {
      ...
      if (hide != null) {
        ...
        patch.authTokens.hide = new RegExp(r'auth=(.+)')
            .firstMatch(hide.attributes['href'])
            ?.group(1);
      }
      if (fave != null) {
        patch.saved = fave.innerHtml.contains('un-fave');
        patch.authTokens.save = new RegExp(r'auth=(.+)')
            .firstMatch(fave.attributes['href'])
            ?.group(1);
      }
      if (reply != null) {
  ...
  Future<int> replyToItemById(int parentId, String comment, String authToken,
    ...
    final req = await (await _httpClient
            .postUrl(Uri.parse('${this._config.apiHost}/comment'))
          ..cookies.add(accessCookie)
  ...
  Future<int> postItem(
    ...
    final req = await (await _httpClient
            .postUrl(Uri.parse('${this._config.apiHost}/r'))
          ..cookies.add(accessCookie)
          // ..headers.add('cookie', '${accessCookie.name}=${accessCookie.value}')
          ..headers.contentType = new ContentType(
              'application', 'x-www-form-urlencoded',
  ...
  Future<String> getSubmissionAuthToken(Cookie accessCookie) async {
    final req = await (await _httpClient
            .getUrl(Uri.parse('${this._config.apiHost}/submit'))
          ..cookies.add(accessCookie))
        .close();
    ...
    final fnid = new RegExp(r'''<input .*?value="([a-zA-Z0-9])*?".*?>''')
        .allMatches(body);
